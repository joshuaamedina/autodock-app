...
MV2_ENABLE_AFFINITY=0 ibrun singularity exec vina_1.2.3.sif python3 autodock.py -r ${receptor} -c "${center_x},${center_y},${center_z}" -s "${size_x},${size_y},${size_z}" -m ${forcefield} -d ${docking} -ll ${library} -n ${top_n_scores} -f $flex